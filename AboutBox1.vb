...
Public NotInheritable Class AboutBox1
    ...
    Private Sub refreshScriptDef_Click(sender As Object, e As EventArgs) Handles refreshScriptDef.Click
        ...
        Else
            If UBound(Scriptcalldefnames) = -1 Then
                PyAddin.UserMsg("no ScriptDefinitions found for ScriptAddin in current Workbook (3 column named range (type/value/path), minimum types: scriptexec and script)!", True)
            Else
                PyAddin.UserMsg("refreshed ScriptDefnames from current Workbook !", True)
            End If
        End If
    End Sub
    ...
    Public Sub checkForUpdate(doUpdate As Boolean)
        ...
        Dim localUpdateFolder As String = PyAddin.fetchSetting("localUpdateFolder", "")
        Dim localUpdateMessage As String = PyAddin.fetchSetting("localUpdateMessage", "A new version is available in the local update folder, after quitting Excel (is done next) start deployAddin.cmd to install it.")
        Dim updatesMajorVersion As String = PyAddin.fetchSetting("updatesMajorVersion", "1.0.0.")
        Dim updatesDownloadFolder As String = PyAddin.fetchSetting("updatesDownloadFolder", "C:\temp\")
        Dim updatesUrlBase As String = PyAddin.fetchSetting("updatesUrlBase", "https://github.com/rkapl123/ScriptAddin/archive/refs/tags/")
        ...
        Catch ex As Exception
            PyAddin.UserMsg("Error setting the SecurityProtocol: " + ex.Message())
            Exit Sub
        End Try
        ...
        If localUpdateFolder <> "" Then
            Try
                If PyAddin.QuestionMsg(localUpdateMessage, MsgBoxStyle.OkCancel) = MsgBoxResult.Ok Then
                    System.Diagnostics.Process.Start("explorer.exe", localUpdateFolder)
                    Me.quitExcelAfterwards = True
                    Me.Close()
                End If
            Catch ex As Exception
                PyAddin.UserMsg("Error when opening local update folder: " + ex.Message())
            End Try
            Exit Sub
        End If
        ...
        Catch ex As Exception
            PyAddin.UserMsg("Couldn't create file download folder (" + updatesDownloadFolder + "): " + ex.Message())
            Exit Sub
        End Try
        ...
        Catch ex As Exception
            PyAddin.UserMsg("Error when downloading new version: " + ex.Message())
            Exit Sub
        End Try
        ...
        Catch ex As Exception
            PyAddin.UserMsg("Error when extracting new version: " + ex.Message())
        End Try
        ...
        Catch ex As Exception
            PyAddin.UserMsg("Error when opening Distribution folder of new version: " + ex.Message())
        End Try
    End Sub
