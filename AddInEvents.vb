...
Public Class AddInEvents
    ...
    Public Sub AutoOpen() Implements IExcelAddIn.AutoOpen
        ...
        If Wb IsNot Nothing Then
            ...
            ElseIf errStr <> vbNullString Then
                PyAddin.UserMsg("Error when getting definitions in AutoOpen: " + errStr, True, True)
            End If
        End If
        ...
        For Each tModule As Diagnostics.ProcessModule In Diagnostics.Process.GetCurrentProcess().Modules
            PyAddin.UserSettingsPath = tModule.FileName
            If PyAddin.UserSettingsPath.ToUpper.Contains("PYADDIN") Then
                PyAddin.UserSettingsPath = Replace(UserSettingsPath, ".xll", "User.config")
                Exit For
            End If
        Next
    End Sub

    ''' <summary>clean up when Pyaddin is deactivated</summary>
    ...
    Private Sub Workbook_Save(Wb As Workbook, ByVal SaveAsUI As Boolean, ByRef Cancel As Boolean) Handles Application.WorkbookBeforeSave
        ...
        If PyAddin.dropDownSelected Then
            errStr = PyAddin.getScriptDefinitions()
    ...
    ''' <summary>refresh ribbon with current workbook's PyAddin Names</summary>
    Private Sub Workbook_Activate(Wb As Workbook) Handles Application.WorkbookActivate
        ...
        PyAddin.theRibbon.Invalidate()
    End Sub

    ''' <summary>get PyAddin Names of current workbook and load ScriptDefinitions of first name in PyAddin Names</summary>
    Private Function doDefinitions(Wb As Workbook) As String
        ...
        End If
        ' get the definitions from the current defined range (first name in PyAddin Names)
        errStr = PyAddin.getScriptDefinitions()
        If errStr <> vbNullString Then Return "Error during getScriptDefinitions in doDefinitions: " + errStr
        LogInfo("done PyDefinitions for workbook " + Wb.Name)
        Return vbNullString
    End Function

    ''' <summary>Close Workbook: remove references to current Workbook and Py Definitions</summary>
