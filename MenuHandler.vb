            ...
            "<group id='PyaddinGroup' label='General settings'>" +
              ...
              "<buttonGroup id='butGrp'>" +
                "<menu id='configMenu' label='Settings'>" +
                  ...
                  "<button id='user' label='User settings' onAction='showAddinConfig' imageMso='ControlProperties' screentip='Show/edit user settings for Py Addin' />" +
                  "<button id='central' label='Central settings' onAction='showAddinConfig' imageMso='TablePropertiesDialog' screentip='Show/edit central settings for Py Addin' />" +
                  "<button id='addin' label='PyAddin settings' onAction='showAddinConfig' imageMso='ServerProperties' screentip='Show/edit standard Addin settings for Py Addin' />" +
                "</menu>" +
                ...
                "<button id='showLog' label='Log' tag='4' screentip='shows Pyaddins Diagnostic Display' getImage='getLogsImage' onAction='clickShowLog'/>" +
              "</buttonGroup>" +
            "<dialogBoxLauncher><button id='dialog' label='About Pyaddin' onAction='refreshScriptDefs' tag='5' screentip='Show Aboutbox (and refresh PyDefinitions from current Workbook from there)'/></dialogBoxLauncher></group>" +
            "<group id='PyScriptGroup' label='Run Scripts defined in WB/sheet names'>"
        Dim presetSheetButtonsCount As Integer = Int16.Parse(PyAddin.fetchSetting("presetSheetButtonsCount", "15"))
...
#Disable Warning IDE0060 ' Hide not used Parameter warning as this is very often the case with the below callbacks from the ribbon
    ...
    Public Sub showAddinConfig(control As IRibbonControl)
        ...
        If InStr(PyAddin.fetchSetting("disableSettingsDisplay", ""), control.Id) > 0 Then
            PyAddin.UserMsg("Display of " + control.Id + " settings disabled !", True, True)
            Exit Sub
        End If
    ...
    Public Sub startScript(control As IRibbonControl)
        ...
        PyAddin.ScriptDefinitionRange = PyAddin.ScriptDefsheetColl(control.Tag).Item(control.Id)
        ...
        Catch ex As Exception
            PyAddin.UserMsg("Selection of worksheet of Script Definition Range not possible (probably because you're editing a cell)!", True, True)
        End Try
        PyAddin.ScriptDefinitionRange.Select()
        errStr = PyAddin.startScriptprocess()
        origSelection.Parent.Select()
        origSelection.Select()
        If errStr <> "" Then PyAddin.UserMsg(errStr, True, True)
        turnOffDesignMode()
    End Sub
    ...
    Private Sub createCButton(sheetName As String, buttonName As String)
        ...
        Try
            cbName = PyAddin.ScriptDefinitionRange.Name.Name
        Catch ex As Exception
        End Try
    ...
    Public Function getImage(control As IRibbonControl) As String
        If PyAddin.debugScript And control.Id = "debug" Then
            Return "AcceptTask"
        Else
            Return "DeclineTask"
        End If
    ...
    Public Function GetDebugLabel(control As IRibbonControl) As String
        ...
        For Each c As Integer In PyAddin.ScriptRunDic.Keys
            If PyAddin.ScriptRunDic(c) Then
                scriptRunning = c
                Exit For
            End If
        Next
        Return "script output " + IIf(PyAddin.debugScript, "active", "inactive") + IIf(scriptRunning < 0, "", " for run: " + CStr(scriptRunning))
    End Function
    ...
    Public Sub toggleButton(control As IRibbonControl, pressed As Boolean)
        If control.Id = "debug" Then
            PyAddin.debugScript = pressed
            PyAddin.setUserSetting("debugScript", pressed.ToString())
            If Not IsNothing(PyAddin.theScriptOutput) Then
                If pressed Then
                    PyAddin.theScriptOutput.Opacity = 1.0
                    'ScriptAddin.theScriptOutput.BringToFront()
                    PyAddin.theScriptOutput.Refresh()
                Else
                    PyAddin.theScriptOutput.Opacity = 0.0
                End If
            End If
            ' invalidate to reflect the change in the toggle buttons image
            PyAddin.theRibbon.InvalidateControl(control.Id)
        End If
        turnOffDesignMode()
    End Sub
    ...
    Public Sub selectItem(control As IRibbonControl, id As String, index As Integer)
        ...
        PyAddin.dropDownSelected = True
        PyAddin.ScriptDefinitionRange = Scriptcalldefs(index)
        PyAddin.ScriptDefinitionRange.Parent.Select()
        PyAddin.ScriptDefinitionRange.Select()
        turnOffDesignMode()
    End Sub
    ...
    Public Function getSheetLabel(control As IRibbonControl) As String
        ...
        If PyAddin.ScriptDefsheetMap.ContainsKey(control.Id) Then getSheetLabel = PyAddin.ScriptDefsheetMap(control.Id)
    End Function
    ...
    Public Function getDynMenContent(control As IRibbonControl) As String
        ...
        Dim currentSheet As String = PyAddin.ScriptDefsheetMap(control.Id)
        For Each nodeName As String In PyAddin.ScriptDefsheetColl(currentSheet).Keys
    ...
    Public Function getDynMenVisible(control As IRibbonControl) As Boolean
        Return PyAddin.ScriptDefsheetMap.ContainsKey(control.Id)
    End Function
