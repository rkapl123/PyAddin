...
''' <summary>Dialog used to display and edit the three parts of PyAddin settings (Addin level, user and central)</summary>
Public Class EditSettings
    ...
    Private Sub EditSettings_Shown(sender As Object, e As EventArgs) Handles Me.Shown
        ' show PyAddin settings (user/central/addin level): set the appropriate config xml into EditBox (depending on Me.Tag)
        ' find path of xll:
        For Each tModule As ProcessModule In Process.GetCurrentProcess().Modules
            Dim sModule As String = tModule.FileName
            If sModule.ToUpper.Contains("PYADDIN") Then
                settingsPath = tModule.FileName
                Exit For
            End If
        Next
        ' read setting from xll path + ".config": addin level settings
        Me.Text = "PyAddin.xll.config settings in " + settingsPath
        Dim settingsStr As String
        Try
            settingsPath += ".config"
            settingsStr = File.ReadAllText(settingsPath, System.Text.Encoding.Default)
        Catch ex As Exception
            PyAddin.UserMsg("Couldn't read PyAddin.xll.config settings from " + settingsPath + ":" + ex.Message, True, True)
            Exit Sub
        End Try
        ' if central or user settings were chosen, overwrite settingsStr
        If Me.Tag = "central" Or Me.Tag = "user" Then
            ' get central settings filename from PyAddin.xll.config appSettings file attribute or
            ' get user settings filename from PyAddin.xll.config UserSettings configSource attribute 
            ...
            If Not IsNothing(doc.SelectSingleNode(xpathStr)) Then
                ...
                If InStr(settingfilename, "\") = 0 Then settingfilename = Replace(settingsPath, "PyAddin.xll.config", "") + settingfilename
                ' and read central settings
                settingsPath = settingfilename
                Try
                    settingsStr = File.ReadAllText(settingsPath, System.Text.Encoding.Default)
                Catch ex As Exception
                    PyAddin.UserMsg("Couldn't read Py Add-in " + Me.Tag + " settings from " + settingsPath + ":" + ex.Message, True, True)
                    Exit Sub
                End Try
                Me.Text = Me.Tag + " settings in " + settingsPath
            Else
                PyAddin.UserMsg("No attribute available as filename reference to " + Me.Tag + " settings (searched xpath: " + xpathStr + ") !", True, True)
                Exit Sub
            End If
        End If
        Me.OKBtn.Text = "Save"
    ...
    Private Sub OKBtn_Click(sender As Object, e As EventArgs) Handles OKBtn.Click
        ...
        Try
            ...
            If Me.Tag = "central" Then schemaString = My.Resources.SchemaFiles.PyAddinCentral
            If Me.Tag = "user" Then schemaString = My.Resources.SchemaFiles.PyAddinUser
        ...
        Catch ex As Exception
            PyAddin.UserMsg("Problems with parsing changed " + Me.Tag + " settings: " + ex.Message, True, True)
            Exit Sub
        End Try
        Try
            File.WriteAllText(settingsPath, Me.EditBox.Text, System.Text.Encoding.UTF8)
        Catch ex As Exception
            PyAddin.UserMsg("Couldn't write " + Me.Tag + " settings into " + settingsPath + ": " + ex.Message, True, True)
            Exit Sub
        End Try
        If Me.Tag = "addin" Then PyAddin.UserMsg("Restart Addin (or Excel) to reflect changes in Addin settings.", True, False)
